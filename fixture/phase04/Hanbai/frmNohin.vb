...
' 2009/02/16 山田 金額を丸めずに持つようにした。丸めは締めのときに合計へ掛ける
' 2009/02/20 山田 締め済みの納品は取り消せないようにした
'============================================================
...
#Region " ボタン処理 "
    ...
    Private Sub btnTorikeshi_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnTorikeshi.Click
        ...
        Try
            ...
            '2009/02/20 山田 締めた後に消されると請求書と合わなくなるので止める
            strSql = "SELECT ISNULL(SEIKYU_NO, '') FROM T_NOHIN WHERE NOHIN_NO = '" & EscQuote(mstrNohinNo) & "'"
            cm = New SqlCommand(strSql, cn)
            strSeikyuNo = ToStr(cm.ExecuteScalar())

            If strSeikyuNo <> "" Then
                MsgBox("この納品は請求書 " & strSeikyuNo & " で締め済みです。" & vbCrLf & _
                       "先に締め処理でその請求を取り消してください。", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
...
#Region " 内部処理 "
    ...
    '------------------------------------------------------------
    '金額計算
    '2009/02/16 山田 1行ずつ丸めると経理のExcelと合わないため、ここでは丸めない。
    '                 丸めは締めのときに月の合計へ掛ける
    '------------------------------------------------------------
    Private Sub KingakuKeisan()
        ...
        txtKingaku.Text = CStr(dblKingaku)
