...
' 2009/03/16 山田 開始残高を追加。Excelから移ってくるときの前回請求額を入れる
'============================================================
    ...
    Private Sub btnToroku_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnToroku.Click
        ...
        Try
            ...
            If mstrTokuisakiCd = "" Then
                ...
                strSql = strSql & "(TOKUISAKI_CD, TOKUISAKI_NM, SEIKYUSAKI_CD, SHIMEBI, SHIHARAI, MARUME_KBN, SHIYO_KBN, KAISHI_ZAN, BIKO, KOSHIN_USER, KOSHIN_DT) "
                ...
                strSql = strSql & "'0',"
                strSql = strSql & CStr(Val(txtKaishiZan.Text)) & ","
            ...
            Else
                ...
                strSql = strSql & "KAISHI_ZAN = " & CStr(Val(txtKaishiZan.Text)) & ","
...
#Region " 一覧 "
    ...
    Private Sub HyojiIchiran()
        ...
        Try
            ...
            strSql = strSql & "ISNULL(T.KAISHI_ZAN, 0) AS KAISHI_ZAN, "
            ...
            dgvIchiran.Columns(8).HeaderText = "開始残高"
            dgvIchiran.Columns(9).HeaderText = "備考"
    ...
    Private Sub dgvIchiran_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvIchiran.CellClick
        ...
        txtKaishiZan.Text = ToStr(r.Cells(8).Value)
        txtBiko.Text = ToStr(r.Cells(9).Value)
...
#Region " 内部処理 "
    ...
    Private Function Check() As Boolean
        ...
        If Trim(txtKaishiZan.Text) <> "" And IsNumeric(txtKaishiZan.Text) = False Then
            MsgBox("開始残高が数字ではありません。", MsgBoxStyle.Exclamation)
            txtKaishiZan.Focus()
            Return False
        End If
