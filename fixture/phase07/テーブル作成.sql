...
-- 2012/02/13 山田 タイの納品のテーブルを追加
-- 2013/05/13 山田 タイの締めのテーブルを追加、タイの納品に締めNoを追加
--
...
--------------------------------------------------------------
-- 2012/02/13 山田 タイの納品
-- 日本の納品(T_NOHIN)とは別に持つ。
-- 金額がバーツで、T_NOHIN は締めと請求書と入金が見ているため。
-- 金額は丸めずに数量×単価をそのまま入れる。
-- タイの締めをどうするかはまだ決まっていない。
--------------------------------------------------------------
CREATE TABLE T_TH_NOHIN (
    NOHIN_NO        varchar(10)     NOT NULL,
    NOHINBI         datetime        NOT NULL,
    DENPYO_NO       varchar(20)     NOT NULL,   --タイの受注の伝票番号
    TOKUISAKI_CD    varchar(10)     NOT NULL,
    HINBAN          varchar(20)     NULL,
    HINMEI          varchar(40)     NULL,
    SURYO           int             NOT NULL,
    TANKA           decimal(11,2)   NOT NULL,   --受注の単価を初期値にするが打ち直せる
    KINGAKU         decimal(13,2)   NOT NULL,   --数量×単価 ※丸めていない
    TSUKA           varchar(3)      NULL,       --受注の通貨をそのまま入れる
    TORIKESHI_KBN   char(1)         NOT NULL,   --0:通常 1:取消
    BIKO            varchar(100)    NULL,
    KOSHIN_USER     varchar(20)     NULL,
    KOSHIN_DT       datetime        NULL,
    CONSTRAINT PK_T_TH_NOHIN PRIMARY KEY (NOHIN_NO)
)
GO

--------------------------------------------------------------
-- 2013/05/13 山田 タイの締め
-- タイの分も月ごとの合計を出して、締めたら数字が動かないようにしたい(経理 田島さん)。
-- 請求書は向こうで出しているので作らない。入金も無い。
-- 現地の締めは月末とのこと(営業 中村さん)なので、月末で全得意先をまとめて締める。
-- 得意先ごとに1件できる。月の合計はこの表を足したもの。
-- 金額は得意先ごとの月の合計に四捨五入を1回だけ掛けたバーツ。
-- 納品(T_TH_NOHIN)のほうは小数2桁のまま
--------------------------------------------------------------
CREATE TABLE T_TH_SHIME (
    SHIME_NO        varchar(10)     NOT NULL,   --締めNo 000001 の形
    SHIME_YM        varchar(6)      NOT NULL,   --締めた年月 YYYYMM
    TOKUISAKI_CD    varchar(10)     NOT NULL,
    KAISHIBI        datetime        NOT NULL,   --月の1日
    SHIMEBI         datetime        NOT NULL,   --月末日
    KENSU           int             NOT NULL,   --納品の件数
    SURYO           int             NOT NULL,   --数量の合計
    KINGAKU         decimal(13,0)   NOT NULL,   --合計に四捨五入を掛けた後 バーツ
    TORIKESHI_KBN   char(1)         NOT NULL,   --0:通常 1:締め取消
    KOSHIN_USER     varchar(20)     NULL,
    KOSHIN_DT       datetime        NULL,
    CONSTRAINT PK_T_TH_SHIME PRIMARY KEY (SHIME_NO)
)
GO

--------------------------------------------------------------
-- 2013/05/13 山田 タイの納品に締めNoを持たせる
-- ※締めたときに入れる。空なら未締め。日本の納品の SEIKYU_NO と同じ使い方。
--   これが入っている納品は取り消せない
--------------------------------------------------------------
ALTER TABLE T_TH_NOHIN ADD SHIME_NO varchar(10) NULL
GO
