...
' 2013/05/13 山田 タイ締めを作ったので、締め済みの月には登録も取消もできないようにした。
'                 締めた後に数字が動くと困るとのこと(経理 田島さん)
'============================================================
...
#Region " ボタン処理 "
    ...
    Private Sub btnToroku_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnToroku.Click
        ...
        Try
            ...
            '2013/05/13 山田 締めた月に入れると締めの数字と合わなくなるので止める
            strSql = "SELECT COUNT(*) FROM T_TH_SHIME WHERE SHIME_YM = '" & Format(dtpNohinbi.Value, "yyyyMM") & "' AND TORIKESHI_KBN = '0'"
            cm = New SqlCommand(strSql, cn)

            If Val(CStr(cm.ExecuteScalar())) > 0 Then
                MsgBox("納品日 " & Format(dtpNohinbi.Value, "yyyy年M月") & " の分はタイ締めで締め済みです。" & vbCrLf & _
                       "納品日を確かめてください。この月に入れるなら、先にタイ締めでその月の締めを取り消してください。", MsgBoxStyle.Exclamation)
                dtpNohinbi.Focus()
                mblnShori = False
                Exit Sub
            End If
    ...
    Private Sub btnTorikeshi_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnTorikeshi.Click
        ...
        Try
            ...
            '2013/05/13 山田 締めた後に消されると締めの数字と合わなくなるので止める
            strSql = ""
            strSql = strSql & "SELECT ISNULL(S.SHIME_YM, '') FROM T_TH_NOHIN N "
            strSql = strSql & "INNER JOIN T_TH_SHIME S ON S.SHIME_NO = N.SHIME_NO "
            strSql = strSql & "WHERE N.NOHIN_NO = '" & EscQuote(mstrNohinNo) & "'"
            cm = New SqlCommand(strSql, cn)
            strShimeYm = ToStr(cm.ExecuteScalar())

            If strShimeYm <> "" Then
                MsgBox("この納品は " & Mid(strShimeYm, 1, 4) & "年" & CStr(Val(Mid(strShimeYm, 5, 2))) & "月 のタイ締めで締め済みです。" & vbCrLf & _
                       "先にタイ締めでその月の締めを取り消してください。", MsgBoxStyle.Exclamation)
                Exit Sub
            End If
