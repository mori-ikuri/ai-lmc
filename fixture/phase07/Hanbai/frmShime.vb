...
' 2014/02/24 山田 消費税を8%にした(平成26年4月1日から)。税率は納品日で決める。
'                 20日締め・25日締めは4月の締めの期間が3月と4月にまたぐので、
'                 3/31までの納品分と4/1からの納品分を分けて税を出して足している。
'                 税の計算は ShohizeiOf() にまとめた
'============================================================
...
#Region " ボタン処理 "
    ...
    Private Sub btnShime_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnShime.Click
        ...
        Try
            ...
            '2014/02/24 山田 税率が変わったので日付で決めるようにした
            dblZei = ShohizeiOf(cn, strJoken, dblUriage, strMarumeKbn, datKaishibi, datShimebi)
...
#Region " 内部処理 "
    ...
    '------------------------------------------------------------
    '消費税
    '2014/02/24 山田 平成26年4月1日から8%。税率は納品日で決める。
    '                 期間が3/31と4/1をまたぐ締め(20日締め・25日締め)は、
    '                 3/31までの納品分に5%、4/1からの納品分に8%を掛けて足す。
    '                 売上額の丸めは今までどおり合計に1回だけ掛けたいので、
    '                 3/31までの分に丸めを掛けて、売上額からそれを引いた残りを
    '                 4/1からの分にしている。端数は今までどおり切り捨て
    '------------------------------------------------------------
    Private Function ShohizeiOf(ByVal cn As SqlConnection, ByVal strJoken As String, ByVal dblUriage As Double, ByVal strMarumeKbn As String, ByVal datKaishibi As Date, ByVal datShimebi As Date) As Double

        Dim cm As SqlCommand
        Dim strSql As String
        Dim dblUriageMae As Double
        Dim dblUriageAto As Double

        '期間の頭と終わりで税率が同じなら、そのまま掛ける
        If ZeiritsuOf(datKaishibi) = ZeiritsuOf(datShimebi) Then
            Return Int(dblUriage * ZeiritsuOf(datShimebi))
        End If

        '税率が変わる日より前の納品分 ※対象の条件は締めるときと同じものを使う
        strSql = ""
        strSql = strSql & "SELECT ISNULL(SUM(N.KINGAKU), 0) FROM T_NOHIN N "
        strSql = strSql & "INNER JOIN M_TOKUISAKI T ON T.TOKUISAKI_CD = N.TOKUISAKI_CD "
        strSql = strSql & strJoken
        strSql = strSql & " AND N.NOHINBI < '" & ZEI_KAISEIBI & "'"

        cm = New SqlCommand(strSql, cn)

        dblUriageMae = Marume(Val(CStr(cm.ExecuteScalar())), strMarumeKbn)
        dblUriageAto = dblUriage - dblUriageMae

        Return Int(dblUriageMae * ZeiritsuOf(datKaishibi)) + Int(dblUriageAto * ZeiritsuOf(datShimebi))
