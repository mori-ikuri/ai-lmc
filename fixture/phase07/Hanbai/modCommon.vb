...
Module modCommon
    ...
    '消費税の税率が変わる日 ※平成26年4月1日から8%
    '2014/02/24 山田 締めの期間がこの日をまたぐときに納品を分けるのにも使う
    Public Const ZEI_KAISEIBI As String = "2014/04/01"
    ...
    '------------------------------------------------------------
    '消費税の税率を返す
    '2014/02/24 山田 平成26年4月1日から5%が8%になる。
    '                 締め取消して締め直したときに過去の月が8%になると困るので、
    '                 今日ではなく渡した日付で決める
    '------------------------------------------------------------
    Public Function ZeiritsuOf(ByVal datHi As Date) As Double

        If datHi >= CDate(ZEI_KAISEIBI) Then
            Return 0.08
        End If
